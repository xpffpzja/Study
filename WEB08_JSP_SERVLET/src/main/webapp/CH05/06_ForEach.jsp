...
<%
	...
	// 배열을 만들고 만든 배열을 request의 Attribute에 저장함.
	request.setAttribute("mList", movieList);
	// 서버에서 JSP 파일로 전달되는 배열이나 ArrayList 보통 request의 Attribute에 담겨옴.
	// 이를 바로 반복실행문에 적용하는 형식이 주로 사용됨.
	// 그래서 배열을 만들고 만든 배열을 request에 저장
...
<!-- 
${state.first} : 현재 반복이 첫번째면 true 리턴
...
<h3>
<c:forEach var="cnt" begin="1"  end="10" varStatus="state">
	${cnt}<c:if test="${not state.last}">, </c:if> <!-- 마지막 데이터가 아니면 , 표시  -->
</c:forEach>
</h3>

<br><br>
<table border="1" style="width:50%; text-align:center;" align="left">
	<tr><th>index</th><th>count</th><th>cnt</th>
